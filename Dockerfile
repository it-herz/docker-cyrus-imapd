...
RUN cd /opt && apt update -y && apt install -y wget autoconf libtool pkg-config libdb5.3++-dev libssl-dev libsasl2-dev libldap2-dev libmysql++-dev libpcre++-dev libjansson-dev libevent-dev make && \
    ...
    mkdir -p /var/lib/cyrus && chown cyrus -R /var/lib/cyrus && mkdir -p /var/lib/cyrus/db && chmod 777 -R /var/lib/cyrus/db && mkdir -p /var/mail && chown cyrus -R /var/mail && \
...
RUN apt install -y uuid-dev libgcrypt-dev libestr-dev flex dh-autoreconf bison python-docutils libxml2-dev git python-setuptools re2c && \
